...
export default function MonthlyGoalOverview({ goals, onClose }: Props) {
  const groupedByMonth: { [month: number]: Goal[] } = {};
  const [selectedGoal, setSelectedGoal] = useState<Goal | null>(null);
  const [editMode, setEditMode] = useState(false);
  const [editedGoal, setEditedGoal] = useState<Partial<Goal>>({});

  goals.forEach((goal) => {
    const month = getMonth(parseISO(goal.startDate));
    if (!groupedByMonth[month]) groupedByMonth[month] = [];
    groupedByMonth[month].push(goal);
  });
  ...
  return (
    <div className="fixed inset-0 bg-white z-50 overflow-y-auto p-6">
      <div className="flex justify-between items-center mb-6">
        <h2 className="text-2xl font-bold">📅 Monatsziele (Übersicht)</h2>
