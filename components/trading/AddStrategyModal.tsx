...
export default function AddStrategyModal({ userId }: Props) {
  ...
  const methods = useForm<StrategyPayload>({
    defaultValues: {
      ...
      name: "",
      description: "",
      tag_color: "#4f46e5", // Indigo als Default
    },
  });
  const { handleSubmit, control, setValue, reset, watch } = methods;
  const v = watch();

  // ——— Event-Wireup: Wenn StrategyPills „öffne Builder mit Confluences“ feuert
  React.useEffect(() => {
    const onOpen = (e: any) => {
    ...
    return () => {
      window.removeEventListener("add-strategy:open", onOpen as EventListener);
    };
  }, []);
  ...
  async function onSubmit(values: StrategyPayload) {
    ...
    try {
      // Backend-Route: falls du schon eine hast, passe die URL an.
      ...
      // andere Komponenten updaten
      window.dispatchEvent(new CustomEvent("strategy-created", { detail: { name: payload.name } }));

      // Optional: SWR-Key in StrategyPills invalidieren
      // (StrategyPills hört zusätzlich auf das strategy-created Event)
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Input placeholder="z. B. London Breakout" {...field} />
                  </FormControl>
                  <FormMessage />
                </FormItem>
              )}
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Textarea placeholder="Kurzbeschreibung deiner Strategie…" {...field} />
                  </FormControl>
                  <FormMessage />
                </FormItem>
              )}
