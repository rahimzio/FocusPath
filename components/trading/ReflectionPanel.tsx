...
export default function ReflectionPanel({ trade, onSaved }: Props) {
  const methods = useForm<FormValues>({
    defaultValues: {
      ...
      performanceState: (trade.performanceState as "A" | "B" | "C") || "A",
      tiltDetected: !!trade.tiltDetected,
    },
  });
  ...
  const pieData = [
    ...
    { name: "Fehler", value: Math.max(0, 100 - disciplineScore) },
  ];
  ...
  return (
    <Card className="w-full max-w-full min-w-0 overflow-hidden">
      <CardHeader className="w-full max-w-full min-w-0">
        <CardTitle className="truncate">Reflexion für {trade.symbol}</CardTitle>
      </CardHeader>
                    ...
                    <FormItem className="min-w-0">
                      <FormControl>
                        <div className="flex items-center gap-2">
                          ...
                          />
                          <label htmlFor="tiltDetected" className="text-sm">
                            Tilt erkannt
                          </label>
                        </div>
                      </FormControl>
