...
export default function GamePicker({
  ...
  const [draft, setDraft] = React.useState<{
    label: string; tagsText: string; points: string; active: boolean; game: Game;
  } | null>(null);
        ...
        <CardHeader className="gap-3 w-full max-w-full min-w-0 overflow-hidden">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 min-w-0">
            <div className="min-w-0">
              <CardTitle className="truncate">A/B/C Game – Auswahl</CardTitle>
              <CardDescription className="truncate">
                Hake Punkte an, die du in diesem Trade erfüllt hast. Klick auf einen Punkt → Details & Bearbeiten.
              </CardDescription>
            </div>
            <div className="flex flex-wrap items-center gap-2 min-w-0">
              <Badge className="h-6 px-2 text-xs" variant={grade === "A" ? "default" : grade === "B" ? "secondary" : "outline"}>
                Grade: {grade}
              </Badge>
            ...
            <div className="grid gap-3">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <Select
                    value={draft.game}
                    onValueChange={(v) => setDraft({ ...draft, game: v as Game })}
                  >
