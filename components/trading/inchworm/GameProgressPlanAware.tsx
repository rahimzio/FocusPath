// components/inchworm/GameProgressPlanAware.tsx
"use client";
...
export default function GameProgressPlanAware({ userId }: { userId: string }) {
  ...
  // ⏱️ Sofort-Refresh wenn der Plan gespeichert wird (CustomEvent + LocalStorage-Änderung)
  React.useEffect(() => {
  ...
  const Delta = ({ v }: { v?: number }) =>
    v == null ? null : (
      ...
      >
        {safeNum(v) > 0 ? "▲" : safeNum(v) < 0 ? "▼" : "•"} {safeNum(v)}
      </span>
    );
      ...
      <CardContent className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <div className="rounded-md border p-3">
          ...
          <ul className="list-disc pl-5 text-sm space-y-1">
            <li>S-Game-Quote (Trades, die ausschließlich A-Faktoren erfüllen).</li>
            <li>Standard-R:R gegen tatsächliche R:R Kennzahl (falls vorhanden).</li>
