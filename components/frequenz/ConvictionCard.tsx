...
function TrendBadge({
  ...
  return (
    <span className={`inline-flex items-center gap-1 text-[10px] ${tone}`}>
      <Icon className="h-3 w-3" /> {label}: {Math.abs(value).toFixed(1)}
      {unit}
    </span>
  );
}
...
export default function ConvictionCard({
  ...
  const baseline =
    typeof baseConviction === 'number' ? Math.max(0, Math.min(10, baseConviction)) : null;
  const smooth =
    typeof smoothed?.convictionSmoothed === 'number'
      ? Math.max(0, Math.min(10, smoothed!.convictionSmoothed!))
      : null;
  const today =
    typeof preview?.convictionToday === 'number'
      ? Math.max(0, Math.min(10, preview!.convictionToday!))
      : null;
  ...
  return (
    <div className="rounded-xl border bg-white p-3 sm:p-4">
          ...
          <TooltipProvider>
            <Tooltip>
              ...
              <TooltipContent className="max-w-xs text-xs">
                <p>
                  <b>Was ist das?</b> Conviction ist deine Grundsicherheit. Änderungen werden per EWMA{' '}
                  <i>langsam</i> geglättet.
                </p>
                <p className="mt-1">
                  <b>Maintain/Magnify:</b> Nach Streaks wird der Schritt etwas größer (Magnify); sonst klein
                  (Maintain).
                </p>
              </TooltipContent>
            </Tooltip>
          ...
          {smoothed?.mmState && (
            <span
              className={`text-[10px] px-2 py-0.5 rounded-full border ${
                smoothed.mmState.mode === 'magnify'
                  ? 'bg-emerald-50 text-emerald-700 border-emerald-200'
                  : 'bg-gray-50 text-gray-700 border-gray-200'
              }`}
            >
              {smoothed.mmState.mode === 'magnify' ? 'Magnify' : 'Maintain'} ·{' '}
              {smoothed.mmState.streakPos || 0}↑/{smoothed.mmState.streakNeg || 0}↓
            </span>
          )}
        </div>
      </div>
      ...
      <div className="mt-2 flex flex-wrap gap-2 text-[11px]">
        {baseline != null && (
          <span className="px-2 py-0.5 rounded-full border bg-gray-50 text-gray-700">
            Baseline: <b>{baseline.toFixed(1)}</b>
          </span>
        )}
        {smooth != null && (
          <span className="px-2 py-0.5 rounded-full border bg-black text-white">
            Smoothed: <b>{smooth.toFixed(1)}</b>
          </span>
        )}
        {today != null && (
          <span className="px-2 py-0.5 rounded-full border bg-gray-100 text-gray-700">
            Heute: <b>{today.toFixed(1)}</b>
          </span>
        )}
      </div>
      ...
      <div
        className="mt-3 h-3 w-full rounded bg-gray-100 overflow-hidden"
        role="progressbar"
        aria-label="Conviction"
        aria-valuemin={0}
        aria-valuemax={10}
        aria-valuenow={typeof main === 'number' ? Number(main.toFixed(1)) : undefined}
      >
        <div className="h-3 bg-black" style={{ width: `${(Number(main || 0) / 10) * 100}%` }} />
      </div>

      <p className="mt-3 text-xs text-gray-600">
        Die Zahl basiert auf Baseline + täglichen Micro-Beweisen. Einzeltage wirken minimal; konsistente Streaks
        werden sanft verstärkt.
      </p>
    </div>
  );
}
