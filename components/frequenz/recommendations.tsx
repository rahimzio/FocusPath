...
export function generateFrequencyTasks(snapshot: FrequencySnapshot, profile: FrequencyProfile): FrequencyRecommendation[] {
    const recommendations: FrequencyRecommendation[] = [];
  
    for (const answer of snapshot.answers) {
      const { questionId, gapScore, currentAnswer, idealAnswer } = answer;
  
      if (gapScore < 0.3) continue; // Kein großer Handlungsbedarf
  
      switch (questionId) {
        case "defaultReactionInStress":
          recommendations.push({
            title: "Stressreaktion verbessern",
            description: "Heute kamst du bei Stress aus dem Gleichgewicht. Nimm dir 10 Minuten für bewusste Atmung oder eine Gehmeditation.",
            category: "Körper",
        ...
        case "selfBeliefLevel":
          recommendations.push({
            ...
            description: "Schreib 3 Dinge auf, auf die du stolz bist – und lies sie dir laut vor.",
            category: "Mindset",
        ...
        case "convictionStyle":
          recommendations.push({
            ...
            description: "Triff heute eine Entscheidung bewusst und ohne lange zu zögern. Handle so, wie dein Traum-Ich es tun würde.",
            category: "Verhalten",
        ...
        case "coreEmotions":
          if (Array.isArray(currentAnswer) && Array.isArray(idealAnswer)) {
            const missing = (idealAnswer as string[]).filter((val) => !(currentAnswer as string[]).includes(val));
            if (missing.length > 0) {
              recommendations.push({
                title: "Emotionen aktivieren",
                description: `Fördere gezielt Emotion(en) wie ${missing.join(", ")}. Z. B. durch Musik, Journal oder Dankbarkeitsübung.`,
                category: "Emotion",
