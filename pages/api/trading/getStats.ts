...
function calcFromTo(range: "week" | "month" | "all", fromQ?: string | null, toQ?: string | null) {
  ...
  const d = new Date(to);
  if (range === "week") d.setUTCDate(d.getUTCDate() - 6); else d.setUTCDate(d.getUTCDate() - 29);
  ...
  try {
    ...
    try {
      await Promise.all([
        ...
        col.createIndex({ userId: 1, createdAt: 1 }), // neu
    ...
    // Basis-Match (tolerant + sauber kombiniert)
    ...
    if (status === "draft") andConds.push({ $or: [{ status: "draft" }, { completed: { $ne: true } }] });
    else if (status === "final") andConds.push({ $or: [{ status: "final" }, { completed: true }] });
    ...
    const pipeline: any[] = [
      ...
      // robuste Normalisierung
      {
        $addFields: {
          ...
          _rating: { $convert: { input: "$rating", to: "double", onError: null, onNull: null } }, // vereinfacht/robust
          _resultNorm: {
            $switch: {
              branches: [
      ...
      },
      {
        $addFields: {
          _dateStr: {
            $cond: [{ $ne: ["$_date", null] }, { $dateToString: { format: "%Y-%m-%d", date: "$_date" } }, null],
          },
        },
      },
    ];

    // Datumsfenster via _dateStr
    if (from || to) {
      const cond: any = {};
    ...
    const [agg] = await col.aggregate([
      ...
      {
        $facet: {
          results: [{ $group: { _id: "$_resultNorm", c: { $sum: 1 } } }],
          avgs: [{
            $group: {
              _id: null,
              avgPnl: { $avg: "$_pnl" },
              sumPnl: { $sum: "$_pnl" },
              avgRating: { $avg: { $cond: [{ $ne: ["$_rating", null] }, "$_rating", null] } },
              count: { $sum: 1 },
            },
          }],
          daily: [
    ...
    for (const r of agg?.results ?? []) resultCounts.set(String(r._id || ""), Number(r.c || 0));
    const wins   = resultCounts.get("win")  ?? 0;
    const losses = resultCounts.get("loss") ?? 0;
    const total  = (agg?.avgs?.[0]?.count as number) ?? wins + losses + (resultCounts.get("BE") ?? 0);

    const denom   = wins + losses;
    const winrate = denom > 0 ? wins / denom : 0;

    const avgPnl    = Number.isFinite(agg?.avgs?.[0]?.avgPnl)    ? Number(agg.avgs[0].avgPnl)    : 0;
    const totalPnl  = Number.isFinite(agg?.avgs?.[0]?.sumPnl)    ? Number(agg.avgs[0].sumPnl)    : 0;
    ...
    // History lückenlos auffüllen (nur für week/month oder explizites Fenster)
    ...
    return res.status(200).json({
      ...
      avgPnl,      // Durchschnitt pro Trade
      totalPnl,    // Summe im Zeitraum
      avgRating,   // Durchschnittliches Rating
