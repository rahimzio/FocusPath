...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    const { userId, label, period } = req.body as {
      userId?: string;
      label?: string;
      period?: { start?: string; end?: string };
    };

    if (!userId || !label || !period?.start || !period?.end) {
      return res.status(400).json({ error: "userId, label, period.start, period.end required" });
    }
    ...
    const tradesAgg = await Trades.aggregate([
      ...
      { $project: {
          rMultiple:1, pnl:1, setup:1, session:1, date:1,
          complianceScore:1, game:1, setupValid:1, riskAdhered:1, executionTiming:1
      }},
      { $facet: {
        base: [
          { $group: {
    ...
    const doc = {
      ...
      kpis: {
        ...
        expectancyR: avgR, avgR, maxDD: 0 /* TODO: equity dd nachziehen */, daysTraded: dailies.length,
