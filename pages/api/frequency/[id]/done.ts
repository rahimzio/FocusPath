...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    await appData.updateOne({ _id }, { $set: { status: "done", completedAt } });
    ...
    return res.status(200).json({ message: "Commitment marked as done" });
  } catch (err: any) {
    console.error("Error marking commitment done", err);
